...
argument-hint: "<spec_excel> <spec_docx> <tên tính năng>"
---
...
|---|---|
| UC Excel không map được section TLGP | Auto-skip UC đó, ghi vào report, tiếp tục |
...
| Input | Cách lấy |
|---|---|
| File Excel (requirements) | Argument 1 hoặc tìm trong `spec/` |
| File TLGP (.docx) | Argument 2 hoặc tìm trong `spec/` |
| Tên tính năng | Argument 3 |
...
### 1A. Đọc Excel
- Xác định sheet D7.x.QLRR hoặc sheet chứa Use-case
- Tìm Use-case tương ứng với tính năng được yêu cầu
- Liệt kê tất cả Transaction từ cột "Giao dịch (Transaction)"
...
- Upload evidence + report lên Google Drive
- Xóa file local đã upload
- Cập nhật `pipeline_report.md` với link Drive
