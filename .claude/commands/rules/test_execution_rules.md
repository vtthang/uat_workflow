...
**Trước khi viết API helper — yêu cầu user cung cấp curl mẫu thành công với ĐẦY ĐỦ tất cả fields trong request body.** Không tự suy đoán field name từ UI label hay TypeScript interface — tên field trên UI và tên field trong API thường khác nhau.

```
# Yêu cầu user: "Bạn cung cấp cho tôi 1 curl tạo bản ghi thành công với đầy đủ các trường?"
# Sau đó map 1-1 từ curl payload vào helper code

# SAI — đoán từ UI placeholder "Tên thông báo" → notificationTitle
# ĐÚNG — lấy từ curl payload thực tế → notificationName, notificationPurpose, notificationPayload
```

Nếu chưa có curl → hỏi user trước khi viết helper, không bắt đầu code.
...
- Ghi vào evidence/log: API gọi gì, request, response (xem mục 5) — phục vụ truy vết khi fail.
...
- KHÔNG sửa bất kỳ cell nào ngoài cột kết quả (và cột evidence/note nếu có). KHÔNG đổi thứ tự dòng, không xoá, không reformat.
...
ws.cell(row=r, column=evidence_col).value = "evidence/TC_LOGIN_013/"
...
```typescript
// Setup spec ghi ra
fs.writeFileSync('test-results/fixture.json', JSON.stringify(articles));
// Batch sau đọc vào
const articles = JSON.parse(fs.readFileSync('test-results/fixture.json', 'utf-8'));
```
...
// Log vào evidence
await page.screenshot({ path: `evidence/${tcId}/api_response_${status}.png` });
```
...
node ./node_modules/.bin/playwright test tests/ --headed
```
